...
// ---------------------------------------------------------------------------
// Scheduler (setTimeout + .unref(), meme pattern que clientActivity)
// ---------------------------------------------------------------------------
...
export function startDailySeniorityScheduler(): void {
  if (process.env.SENIORITY_JOB_ENABLED !== 'true') return;

  const hourRaw = Number(process.env.SENIORITY_JOB_HOUR || 1);
  const minuteRaw = Number(process.env.SENIORITY_JOB_MINUTE || 0);
  const hour = Number.isFinite(hourRaw) ? Math.min(23, Math.max(0, Math.trunc(hourRaw))) : 1;
  ...
  async function runAndReschedule(): Promise<void> {
    try {
      console.log('[seniority-job] starting', { hour, minute });
      const res = await runDailySeniorityJob();
      if (res) {
        console.log('[seniority-job] done', {
          runId: res.runId,
          clientsScanned: res.clientsScanned,
          clientsUpdated: res.clientsUpdated,
          clientsSkipped: res.clientsSkipped,
          clientsFailed: res.clientsFailed,
          durationMs: res.finishedAt.getTime() - res.startedAt.getTime()
        });
      }
    } catch (e: any) {
      console.warn('[seniority-job] failed', { error: String(e?.message || e) });
    } finally {
      const ms = msUntilNextLocalTime({ hour, minute, second: 0 });
      console.log('[seniority-job] next run scheduled', { inMs: ms });
      setTimeout(() => void runAndReschedule(), ms).unref();
    }
  }

  const firstDelay = msUntilNextLocalTime({ hour, minute, second: 0 });
  console.log('[seniority-job] scheduler enabled', { hour, minute, firstDelayMs: firstDelay });
