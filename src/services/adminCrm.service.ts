...
export async function createRequestAdmin(payload: Record<string, unknown>, createdBy: string) {
  ...
  const row: Record<string, any> = {
    ...
    tags: payload.tags ?? [],
    waiting_time: payload.waiting_time ?? null,
    is_whatsapp_request: payload.is_whatsapp_request ?? true,
    client_language: payload.client_language ?? null,
    ai_description: payload.ai_description ?? null,
    for_who: payload.for_who ?? null,
    created_at: now,
    updated_at: now,
  };
  ...
  if (userId) {
    ...
    try {
      const firestoreData: Record<string, any> = {
        ...
        'CRM_Request_Status': row.status,
        'CRM_Request_AssignedTo': row.assigned_to,
        'First Name': row.first_name,
        'Last Name': row.last_name,
        'Email': row.email,
        'Membership Type': row.membership_type,
        'CRM_Request_Priority': row.priority,
        'CRM_Request_Difficulty': row.difficulty,
        'Request Date': new Date(row.request_date),
        'createdBy': 'CRM',
        'source': 'CRM',
        'platform': 'web',
        'is_rdv': row.is_rdv,
        'CRM_WaitingTime': row.waiting_time,
        'is_whatsapp_request': row.is_whatsapp_request,
        'client_language': row.client_language,
        'ai_description': row.ai_description,
        'Request_ForWho': row.for_who ? [row.for_who] : [],
        'isRequestOpened': true,
        'CRM_Request_Response': '',
        'CRM_Request_ResponseAttachedFiles': [],
        'Request_ActiveStep': 0,
      };
